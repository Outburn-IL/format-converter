...
import { parseXml } from '../src/converters';

describe('xmlToJson', () => {
  it('parses FHIR nested xml extension correctly', () => {
    const xml: string = `
<address attribute1="foo">
  ...
  it('parses C-CDA observation xml', () => {
    const xml: string = `
<observation classCode="OBS" moodCode="EVN">
  ...
  it('parses namespaced xml with multiple namespaces', async () => {
    const xml: string = `
...
`;
  ...
  it('parses FHIR observation xml', () => {
    const xml: string = `
<Observation xmlns="http://hl7.org/fhir">
  ...
  it('parses xml with declaration tag', () => {
    const xml: string = `
...
<Observation xmlns="http://hl7.org/fhir">
  ...
  describe('invalid xmls', () => {
    it('should throw on malformed xml (missing closing tag)', () => {
      const invalidXml = '<root><child></root>';
      expect(() => parseXml(invalidXml)).toThrow('Expected closing tag');
    });

    it('should throw on malformed xml (unclosed tag)', () => {
      const invalidXml = '<root><child></root';
    ...
    it('should throw on empty string', () => {
      const invalidXml = '';
      expect(parseXml(invalidXml)).toEqual({});
    });
  });
});
